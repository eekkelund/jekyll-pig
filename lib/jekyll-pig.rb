...
module JekyllPig
    ...
    class JekyllPig < Jekyll::Generator
        ...
        def full_size_html(gallery_name, name, date, prev_url, next_url, video)
            "---\n"                                                                                                                                         \
            "layout: post\n"                                                                                                                                \
            "title: #{name}\n"                                                                                                                              \
            ...
            "#{video.nil? ? '<img src="{{site.baseurl}}/assets/img/' + gallery_name + '/1024/' + name + '"/>' : '<video controls src="{{site.baseurl}}/assets/img/' + gallery_name + '/video/' + video + '"/>'}\n"
        end
        ...
        def get_video(image_name, videos)
            basename = File.basename(image_name, File.extname(image_name))
            videos.grep(/(#{basename})\.(mpg|mov|mp4)/).first
        end
        ...
        def process_image(image_data, gallery_id, gallery_path, image_name, videos)
            ...
            if not File.exist? full_size_html_path
                ...
                full_size_html = full_size_html(gallery_id, image_name, image_date, 
                                                ...
                                                get_next_url(image_data, gallery_id, image_name), get_video(image_name,videos))
        ...
        def augment_image_data(gallery, image_data, images, videos) 
            images.each do |image_name|
                ...
                if not image_data.any? { |data| data['filename'] == image_name }
                    video = get_video(image_name,videos)
                    #get image date
            ...
            galleries.each do |gallery|
                ...
                images.each do |image_name|
                    if gallery.create_html
                        ...
                        process_image(image_data, gallery.name, gallery.path, image_name, videos)
                    end
                end
